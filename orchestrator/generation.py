...
from typing import TYPE_CHECKING, Callable, Dict, Mapping, Optional, Sequence, Tuple, cast
...
from .fewshot import FewShotExample, append_example as append_fewshot_example, retrieve_examples
...
class ProgramGenerator:
    ...
    def _materialise_snippet(
        ...
        *,
        ...
        failure_tags = self._extract_failure_tags(failure_context)
        ...
        fewshot_examples = retrieve_examples(problem_id, baseline_block, failure_tags)
        ...
        if fewshot_examples:
            for index, example in enumerate(fewshot_examples, start=1):
                tags_display = ", ".join(example.get("tags", [])) or "none"
                context_lines.extend(
                    [
                        "",
                        f"Few-shot Example #{index} (tags: {tags_display}, intent: {example.get('intent', 'unknown')}, success={bool(example.get('success', False))}):",
                        "SEARCH:",
                        example.get("search", ""),
                        "REPLACE:",
                        example.get("replace", ""),
    ...
    @staticmethod
    def _extract_failure_tags(context: Optional[str]) -> List[str]:
        if not context:
            return []
        tags: List[str] = []
