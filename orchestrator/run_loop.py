...
from .fewshot import FewShotExample, append_example as append_fewshot_example
...
class EvolutionOrchestrator:
    ...
    async def step(self) -> Optional[ProgramCandidate]:
        ...
        self._maybe_append_fewshot_example(candidate, success=candidate.status is EvalStatus.SUCCEEDED)
        LOGGER.info(
    ...
    def _sample_and_generate(self) -> ProgramCandidate:
        ...
        arm_name, prompt = self.prompt_bandit.pick_prompt(
            intent="mutate", problem_id=problem_id, prefer_island="robustness", context=context
        )
    ...
    def _queue_repair_candidate(
        ...
        try:
            context = {
                ...
                "failure_tags": failure_context,
                "complexity_norm": min(candidate.metrics.loc / 200.0, 1.0)
                if candidate.metrics
                else 0.0,
                "invalid_density": min(
                    self.prompt_bandit.arms[candidate.prompt_arm].invalid_responses / 5.0, 1.0
                )
                if candidate.prompt_arm in self.prompt_bandit.arms
                else 0.0,
                "recent_reward": self.prompt_bandit.arms[candidate.prompt_arm].recent_reward
                if candidate.prompt_arm in self.prompt_bandit.arms
                else 0.0,
            }
            arm, prompt = self.prompt_bandit.pick_prompt(
                intent="repair", problem_id=candidate.problem_id, context=context
            )
        except ValueError:
        ...
        self._maybe_append_fewshot_example(candidate, success=False)
    ...
    def _maybe_append_fewshot_example(self, candidate: ProgramCandidate, *, success: bool) -> None:
        metadata = candidate.patch_payload.get("metadata", {})
        if not isinstance(metadata, dict):
            return
        if metadata.get("strategy") != "llm_api":
            return
        search = metadata.get("anchor")
        replace = candidate.patch_payload.get("payload")
        if not isinstance(search, str) or not isinstance(replace, str):
            return
        tags = []
        if not success:
            tags.append("failure")
        tags.extend(candidate.eval_passes)
        example = FewShotExample(
            search=search,
            replace=replace,
            tags=tags,
            intent=metadata.get("intent", candidate.prompt_arm),
            success=success,
        )
        append_fewshot_example(candidate.problem_id, example)
