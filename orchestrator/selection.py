...
class SelectionStrategy:
    ...
    def select_next(
        ...
        context = {
            ...
            "failure_tags": parent.eval_passes,
            "complexity_norm": min(parent.metrics.loc / 200.0, 1.0) if parent.metrics else 0.0,
            "invalid_density": min(
                prompt_bandit.arms[parent.prompt_arm].invalid_responses / 5.0, 1.0
            )
            if parent.prompt_arm in prompt_bandit.arms
            else 0.0,
            "recent_reward": prompt_bandit.arms[parent.prompt_arm].recent_reward
            if parent.prompt_arm in prompt_bandit.arms
            else 0.0,
        }
        if migrate:
    ...
    def bootstrap_population(
        ...
        while len(seeds) < self.population_size:
            ...
            arm_name, prompt = bandit.pick_prompt(intent="mutate", problem_id=problem_id, context=context)
