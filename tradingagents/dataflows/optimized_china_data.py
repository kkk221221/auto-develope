...
"""
    ...
    def get_stock_data(self, symbol: str, start_date: str, end_date: str,
                      force_refresh: bool = False, period: str = "daily") -> str:
        """
        获取A股数据 - 优先使用缓存
        ...
        Args:
            ...
            period: 数据周期（如"daily"、"1min"等）

        Returns:
            格式化的股票数据字符串
        """
        logger.info(f"📈 获取A股数据: {symbol} ({start_date} 到 {end_date}, 周期={period})")
        ...
        if not force_refresh:
            ...
            if adapter.use_app_cache:
                ...
                if df is not None and not df.empty:
                    logger.info(f"📊 [数据来源: MongoDB] 使用MongoDB历史数据: {symbol} ({len(df)}条记录, 周期={period})")
                    return df.to_string()
        ...
        if not force_refresh:
            ...
            if cache_key:
                ...
                if cached_data:
                    logger.info(f"⚡ [数据来源: 文件缓存] 从缓存加载A股数据: {symbol} (周期={period})")
                    return cached_data
        ...
        try:
            ...
            if "❌" in formatted_data or "错误" in formatted_data:
                ...
                # 尝试从旧缓存获取数据
                old_cache = self._try_get_old_cache(symbol, start_date, end_date, period)
                if old_cache:
                ...
                return self._generate_fallback_data(symbol, start_date, end_date, period, "数据源API调用失败")
            ...
            # 生成备用数据
            return self._generate_fallback_data(symbol, start_date, end_date, period, error_msg)
...
- 风险承受能力较低的投资者应避免"""
    ...
    def _try_get_old_cache(self, symbol: str, start_date: str, end_date: str, period: str = "daily") -> Optional[str]:
        """尝试获取过期的缓存数据作为备用"""
        try:
            # 查找任何相关的缓存，不考虑TTL
            for metadata_file in self.cache.metadata_dir.glob(f"*_meta.json"):
                try:
                    ...
                    if (metadata.get('symbol') == symbol and
                        ...
                        metadata.get('market_type') == 'china' and
                        metadata.get('period', 'daily') == period):
    ...
    def _generate_fallback_data(self, symbol: str, start_date: str, end_date: str, period: str, error_msg: str) -> str:
        """生成备用数据"""
...
def get_china_stock_data_cached(symbol: str, start_date: str, end_date: str,
    ...
    return provider.get_stock_data(symbol, start_date, end_date, force_refresh, period)
