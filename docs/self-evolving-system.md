...
- **Few-shot 案例库**：从 `.artifacts/candidates/` 与 Git 谱系沉淀“成功补丁”“失败→修复”对照，按 `problem_id + failure_tags` 建立检索索引（BM25/向量）；每次采样前选取 ≤2 条示例拼接在 prompt `Context` 段，帮助模型快速对齐目标与陷阱，同时保持系统提示的 JSON 约束。
- **组装**：随机化指令顺序增强多样性；目标权重随代数退火；请求载荷附带 `problem_id`、目标文件路径、当前 EVOLVE-BLOCK 源码与上下文特征（失败标签密度、复杂度归一值、invalid 密度、近期奖励等），既给 LinUCB 打分使用，也为 `payload.search` 提供精确锚点。
...
- 算法：**Thompson Sampling + LinUCB**；冷启均匀探索，热启承继历史。上下文向量涵盖问题匹配、失败密度、复杂度归一值、invalid 密度、近期奖励等特征；LinUCB 的探索系数 α 会依据成功/失败动态衰减或回升，结合 invalid 惩罚形成熔断机制。
- 遥测：每个 step 后导出 `.artifacts/prompt_telemetry.json`，记录成功率、温度、checklist、最新奖励、`invalid_responses` 与当前 `linucb_alpha` 等指标，供治理与告警使用。
- 上下文 Bandit：采样时携带 `problem_id`、失败标签、复杂度、invalid 统计等特征，对臂得分进行重加权；连续失败或结构化违规的臂会自动降低探索并强化 checklist 约束。
